...
class TasksListCard extends StatefulWidget {
  ...
  final Function(TasksData)? onTap;

  final bool isOpen;
  final TasksData? drinkData;
  final int earnedPoints;

  const TasksListCard({Key? key, this.drinkData, this.onTap, this.isOpen = false, this.earnedPoints = 100}) : super(key: key);

  @override
  _TasksListCardState createState() => _TasksListCardState();
}

class _TasksListCardState extends State<TasksListCard> with TickerProviderStateMixin {
   bool _wasOpen=false;
  ...
  Widget build(BuildContext context) {
    ...
    var pointsValue = pointsRequired! - _pointsTween.value * min(widget.drinkData!.collectedPoints, pointsRequired!);
    //Determine current fill level, based on _fillTween
    double _maxFillLevel = min(1, widget.drinkData!.collectedPoints / widget.drinkData!.requiredPoints);
    double fillLevel = _maxFillLevel; //_maxFillLevel * _fillTween.value;
    double cardHeight = widget.isOpen ? TasksListCard.nominalHeightOpen : TasksListCard.nominalHeightClosed;
  ...
  Stack _buildLiquidBackground(double _maxFillLevel, double fillLevel) {
    return Stack(
      ...
      children: <Widget>[
        Transform.translate(
          offset:
              Offset(0, TasksListCard.nominalHeightOpen * 1.2 - TasksListCard.nominalHeightOpen * _fillTween.value * _maxFillLevel * 1.2),
          child: CustomPaint(
  ...
  Column _buildBottomContent(double pointsValue) {
    bool isDisabled = widget.drinkData!.collectedPoints < widget.drinkData!.requiredPoints;
    ...
    return Column(
      children: [
        ...
        SizedBox(height: 16),
        Text(
          "Redeem your points for a cup of happiness! Our signature espresso is balanced with steamed milk and topped with a light layer of foam. ",
